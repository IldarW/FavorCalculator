...
public class FavorCalculator implements WurmClientMod, Initable{
    ...
    public static boolean handleInput(final String cmd, final String[] data) {
        switch (cmd) {
            case "favor":
                if (data.length ==  3 && data[1].equals("setfactor")) {
                    try {
                        factor = Float.parseFloat(data[2]);
                        hud.consoleOutput("The calculated favor will be multipled with " + factor);
                ...
                try {
                    ...
                    for(int i = 0; i < components.size(); i++) {
                        ...
                        if (       (wurmComponent instanceof InventoryWindow
                                ||  wurmComponent instanceof  ItemListWindow)
                                &&  wurmComponent.contains(x,y)) {
                            ...
                            for (long target : targets) {
                                float quality = inventoryWindow.getItem(target).getQuality() * (1- inventoryWindow.getItem(target).getDamage()/100);
                                favor += quality * quality / 500 * factor;
                            }
                            String info = String.format("The would probably get %.2f favor for this", favor);
